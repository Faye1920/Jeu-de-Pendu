...
class SuccessViewController: UIViewController {
    ...
    func configureView() {
        ...
        let jeuPendu = JeuPendu.shared
        numberOfAttempts = jeuPendu.getCurrentNbErrors()
        
        if numberOfAttempts < 7 {
            let remainingAttempts = 7 - numberOfAttempts
            let score = remainingAttempts * 100 / 7
    ...
    func startNewGame() {
        ...
        if let filmViewController = storyboard.instantiateViewController(withIdentifier: "FilmViewController") as? FilmViewController {
            navigationController?.setViewControllers([filmViewController], animated: true)
        }
    }
